...
from TDReminder.BERT_model import codeModel2, Config
from TDReminder.codebert_train_old import Data_processor
...
def test_model(dir_path, epoch_id, keyword):
    ...
    model_state_path = '../autodl-tmp/codebert_'+ keyword +'_newpython_old_maxlen512/epoch_' + str(epoch_id) + '/model.ckpt'
    print(model_state_path)
    # load model
    model = codeModel2(config).to(config.device)
    ...
    # output to csv
    output_path = model_state_path[:-18] + 'codebert_newpython_todo_prob.csv'
    ...
    # todo
    write_file('./output_dir/tdreminder_' + keyword +'_newpython_maxlen512_old_' + str(epoch_id), all_pre_label)
...
if __name__ == '__main__':
    ...
    for epoch_id in range(6, 7):
        test_model(dir_path, epoch_id, 'sbp')
